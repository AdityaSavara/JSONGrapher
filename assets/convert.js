...
function Convert() {
  ...
  this.convert = function (input, target) {
    ...
    const isTargetCurly =
      isTarget && Array.isArray(tObj[0]) && tObj[0][0] === "{}"; //whether curly is used in target (appropriately!)
  ...
  this.checkPrefix = function (pref, unit) {
    ...
    let word = false;
    (!unit.prefix || unit.prefix === "0") &&
      (word = this.msgDB["WARN_prefixes_word0"]);
    unit.prefix === "+" &&
      pref.v < 0 &&
      (word = this.msgDB["WARN_prefixes_word+"]);
    unit.prefix === "-" &&
      pref.v > 0 &&
      (word = this.msgDB["WARN_prefixes_word-"]);
  ...
  this.checkDimension = function (source, target) {
    ...
    faults.length > 0 &&
      this.warn(this.msgDB["WARN_target_dim_mismatch"](faults)); //nicely written warning
  ...
  const checkZeros = (v) =>
    v.reduce((acc, o) => acc && Math.abs(o) < dimTolerance, true);
  ...
  this.format = function (output, params) {
    ...
    if (params.exp) {
      ...
      output.num2 =
        dn > dp
          ? (Math.round(num / 10 ** (dn - dp)) * 10 ** (dn - dp)).toFixed(0)
          : num.toPrecision(dp);
    } else {
