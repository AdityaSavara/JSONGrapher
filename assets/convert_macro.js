...
function Convert_macro(code) {
  ...
  try {
    ...
    function cleanupCb(line) {
      ...
      if (line.search("<js>") !== 0 && c > -1) {
        line = line.slice(0, c);
      } //cut the line comment
      return line;
    }
    ...
    function expand(expression) {
      ...
      Object.keys(variables)
        ...
        .forEach(function (v) {
          ...
          if (expression.search(reg) === -1) {
            return;
          }
          const expandedVar = `(${variables[v].n}*${that.vector2text(
            variables[v].v,
            true
          )})`.replace(/\*\)$/, ")");
    ...
    const functions = {
      ...
      convert: {
        argsMin: 1,
        argsMax: 3,
        f: function (input, target, params) {
    ...
    for (let i = 0; i < code.length; i++) {
      ...
      if (eqs.length - 1 >= 2) {
        throw that.msgDB["ERRC_equalSigns"](line);
      }
      if (eqs.length - 1 === 1) {
        const varName = eqs[0]; //left side of assignment
        if (!checkVarName(varName)) {
          throw that.msgDB["ERRC_varName"](line, varName);
        }
      ...
      //line is contains a function call, try to identify it it
        ...
        for (let v of Object.keys(functions)) {
          ...
          const errCond =
            (f.hasOwnProperty("argsMin") && args.length < f.argsMin) ||
            (f.hasOwnProperty("argsMax") && args.length > f.argsMax);
          if (errCond) {
            throw that.msgDB["ERRC_argCount"](
              line,
              v,
              f.argsMin || 0,
              f.argsMax || "n",
        ...
        if (!found) {
          throw that.msgDB["ERRC_unreadableLine"](line);
        }
      }
    }
  } catch (err) {
