...
function Convert_parse(convert, text) {
  ...
  Units.forEach(
    (o) => o.alias && o.alias.forEach((a) => UnitIdMap.push({ id: a, ref: o }))
  ); //and push all aliases
  ...
  function protectNumbers(text, IACB) {
    ...
    m &&
      m.forEach(
        (m) =>
          (text = text.replace(m, m.replace(/\-/g, "~").replace(/\+/g, "#")))
      );
    return text;
  }
