...
function applyTraceStyleToSingleDataSeries(dataSeries, traceStylesCollection = "", traceStyleToApply = "") {
  ...
  if (
    typeof traceStyleToApply === "string" &&
  ...
  if (typeof traceStyleToApply === "string") {
    if (
      traceStyleToApply.toLowerCase() === "nature" ||
      traceStyleToApply.toLowerCase() === "science"
    ) {
...
/**
 * Determines the appropriate colorscale structure based on the trace style.
 * For bubble plots, prepares bubble sizes. For 3D plots, assigns structure type.
 *
 * @param {Object} dataSeries - The Plotly data series to inspect and modify.
 * @param {String} traceStyle - The trace style name (e.g., "bubble2d", "mesh3d").
 * @param {String} colorscale - The colorscale name (not used directly here).
 * @returns {Object} An object containing the updated dataSeries and the inferred colorscaleStructure.
 */
...
/**
 * Determines the appropriate colorscale structure based on the trace mode or type.
 * Used when the structure is still undefined after initial style parsing.
 *
 * @param {Object} dataSeries - The Plotly data series to inspect.
 * @param {String} traceStyle - The trace style name (not used directly here).
 * @param {String} colorscale - The colorscale name (not used directly here).
 * @returns {Object} An object containing the updated dataSeries and inferred colorscaleStructure.
 */
function determineColorScaleStructureSecondHalf(dataSeries, traceStyle, colorscale) {
  ...
  if (
    dataSeries.mode.includes("markers") ||
    dataSeries.mode.includes("markers+lines") ||
    dataSeries.mode.includes("lines+markers")
  ) {
...
/**
 * Applies a Plotly-compatible colorscale to a data series based on its structural type.
 * Handles bubble, 3D, mesh, marker, and line styles, and replaces null values with 0.
 *
 * @param {Object} dataSeries - The Plotly data series to modify.
 * @param {String} colorscale - The name of the colorscale to apply.
 * @param {String} colorscaleStructure - The structural context for applying the colorscale.
 * @returns {Object} The updated data series with colorscale applied.
 */
...
/**
 * Prepares bubble sizes for a Plotly bubble plot by normalizing and scaling marker sizes.
 * Uses z, z_points, y, or bubble_sizes as input, and sets hover text accordingly.
 *
 * @param {Object} dataSeries - The Plotly data series to modify.
 * @returns {Object} The updated data series with normalized and scaled bubble sizes.
 */
...
function applyLayoutStyleToPlotlyDict(figDict, layoutStyleToApply = "default") {
  ...
  // Apply style dictionary to create a fresh layout object // using JSON to create a deep copy of the styleDict.layout
