...
/**
 * Performs a deep copy of a JSON-compatible object.
 * Note: This method does not preserve functions, Dates, Maps, Sets, or circular references.
 *
 * @param {Object} obj - The object to deep copy.
 * @returns {Object} A new object with identical structure and values.
 */
...
/**
 * Merges a new figure dictionary into an existing one, prepares it for plotting,
 * and renders the result using Plotly. Handles optional messaging and error reporting.
 *
 * @param {Object} existingFigDict - The current figure dictionary.
 * @param {Object} newFigDict - The new figure dictionary to merge.
 * @param {String} newFigDictFileName - The filename associated with the new figure.
 * @param {String} graphDivName - The ID of the div where the plot should be rendered.
 * @param {HTMLElement|string|null} messagesToUserDiv - Optional div or message string for user feedback.
 * @param {HTMLElement|null} errorDiv - Optional div for displaying error messages.
 * @returns {Object|undefined} The merged figure dictionary if plotting succeeds; otherwise undefined.
 */
export async function mergeAndplotData(
  existingFigDict,
  newFigDict,
  newFigDictFileName,
  graphDivName,
  ...
  const { mergedFigDict, fileName, newestFigDict } = await prepareForPlotting(
    existingFigDict,
    newFigDict,
    newFigDictFileName,
    errorDiv
  ...
  if (mergedFigDict) {
    ...
    // Note that we don't want to put this message in the errorDiv.
    if (!messagesToUserDiv.innerText) {
      // Currently, we assume the below message is present or not present.
      // If we later put additional messagesToUser, we may need to add more logic.
      const dataPlottedMessage =
        "\u2003\u2003\u2003\u2003\u2003\u2003 Data plotted! Add more data or click 'Clear Data' to start a new graph! \u2003\u2003\u2003\u2003\u2003\u2003";
...
/**
 * Prepares a new figure dictionary for plotting by evaluating implicit operations,
 * extracting units, and merging with an existing figure if present.
 *
 * @param {Object|null} existingFigDict - The current figure dictionary, or null if first upload.
 * @param {Object} newFigDict - The new figure dictionary to process and merge.
 * @param {String} newFigDictFileName - The filename associated with the new figure.
 * @param {HTMLElement|null} errorDiv - Optional div for displaying error messages.
 * @returns {Promise<Object>} An object containing the merged figure, the processed new figure, and the filename.
 */
export async function prepareForPlotting(existingFigDict, newFigDict, newFigDictFileName, errorDiv) {
  ...
  try {
    ...
    throw err;
      ...
      /**
 * Checks compatibility between two figure dictionaries and merges them if valid.
 * Validates datatype and axis labels (excluding units), performs unit conversion,
 * and appends new data to the existing dictionary. Displays errors if incompatible.
 *
 * @param {Object} existingFigDict - The current figure dictionary.
 * @param {Object} newFigDict - The new figure dictionary to validate and merge.
 * @param {HTMLElement} errorDiv - A div element for displaying error messages.
 * @returns {Promise<Object|null>} The merged figure dictionary, or null if incompatible.
 */
async function checkAndMergeFigDict(existingFigDict, newFigDict, errorDiv) {
  ...
  let fieldsMatch = true; // initialize as true, will set false if the fields don't match

  // Check compatibility of datatype
  if (existingFigDict.datatype !== newFigDict.datatype) {
    fieldsMatch = false;
    errorDiv.innerText +=
      "The added record's datatype is different. Stopping merging. The two values are: " +
  ...
  if (
    ...
    fieldsMatch = false;
    errorDiv.innerText +=
      "The added record's xaxis label text is different. Stopping merging. The two values are: " +
      removeUnitFromLabel(existingFigDict.layout.xaxis.title.text) + " " +
      removeUnitFromLabel(newFigDict.layout.xaxis.title.text) + "\n";
  }
  ...
  if (
    ...
    fieldsMatch = false;
    errorDiv.innerText +=
      "The added record's yaxis label text is different. Stopping merging. The two values are: " +
      removeUnitFromLabel(existingFigDict.layout.yaxis.title.text) + " " +
      removeUnitFromLabel(newFigDict.layout.yaxis.title.text) + "\n";
  }
  ...
  if (!fieldsMatch) {
    errorDiv.innerText +=
      `Added data not plotted. The records were not compatible for merging. You may continue trying to add data sets, or may click "Clear Data" to start a new graph. These error messages will be automatically cleared after 10 seconds.\n`;
    setTimeout(() => {
      errorDiv.innerText = "";
    }, 10000);
    return null;
...
/**
 * Prepares and renders a Plotly graph from a figure dictionary.
 * Applies plot styles, executes layout transformations, cleans metadata,
 * and replaces superscripts for compatibility with Plotly.
 *
 * @param {Object} figDict - The figure dictionary to plot.
 * @param {String} [graphDivName="graph1"] - The ID of the div where the plot should be rendered.
 * @returns {Promise<void>} Resolves when the plot is rendered or logs an error if Plotly is unavailable.
 */
async function plot(figDict, graphDivName = "graph1") {
  ...
  if (
    ...
    JSON.stringify({ layout_style: "", trace_styles_collection: "" })
  ) {
    plotStyle = figDict.plot_style ?? {
      layout_style: "",
      trace_styles_collection: "",
    };
  }

  // Plotly mutates the input, and we also do when applying plot style.
  let copyForPlotly = JSON.parse(JSON.stringify(figDict));

  // offset2D and arrange2dTo3d must be executed after making the copy, if requested.
  // The other implicit functions have already been called.
  copyForPlotly = await executeImplicitDataSeriesOperations(
    copyForPlotly,
    false,
    false,
    false,
  ...
  copyForPlotly = cleanJsonFigDict(copyForPlotly, [
    "simulate",
    "custom_units_chevrons",
    "equation",
    "trace_style",
    "3d_axes",
    "bubble",
  ...
  if (copyForPlotly.layout.scene) {
    copyForPlotly.layout.scene.yaxis.title.text = replaceSuperscripts(
      copyForPlotly.layout.scene.yaxis.title?.text ?? ""
    );
    copyForPlotly.layout.scene.xaxis.title.text = replaceSuperscripts(
      copyForPlotly.layout.scene.xaxis.title?.text ?? ""
    );
  } else {
    copyForPlotly.layout.yaxis.title.text = replaceSuperscripts(
      copyForPlotly.layout.yaxis.title?.text ?? ""
    );
    copyForPlotly.layout.xaxis.title.text = replaceSuperscripts(
      copyForPlotly.layout.xaxis.title?.text ?? ""
    );
  }
